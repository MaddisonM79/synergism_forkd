...
### P0.4 — Build the state-snapshot parity harness  *(infrastructure — do before P1)*
- **Build:** serialize a `GameState` → run `tack` (and key aggregators: `compute_global_multipliers`, `update_all_multiplier`, `compute_obtainium`, cube multiplier) → snapshot post-state → diff against a TS oracle over a **non-default, multi-tick** grid. Extend the existing `crates/synergismforkd_testkit/src/parity.rs` + the `.mjs` generators.
...
- **Note:** the current `.mjs` path is restricted to deterministic, RNG-free, dependency-free leaf fns — this needs a **different fixture-capture approach** (drive the monolith/extracted `tackBody` over seeded states, capture resource snapshots). Reuse the RNG-divergence carve-out (don't anchor ambrosia/red-ambrosia luck bit-values).
- **Acceptance:** at least one non-default multi-tick fixture per Phase-1 target lands diffing Rust vs TS within tolerance. **Effort: medium-high.**
...
### P1.2 — Award offerings on every reset tier  *(medium — NOT "pure wiring", audit corrected)*
- **Reality (verified):** `calculate_offerings` is ported but has **zero production callers** (only a test); the auto-trickle writes a *separate* `automation.offerings` slice, not the canonical `runes.rune_shards`. So this needs the `calculateOfferings()` **input-builder** ported — reset-timer (prestige/transcend/reincarnation counter) → `base_offerings` + the offering-multiplier StatLine + `time_multiplier` — then award to `runes.rune_shards` at the start of each `perform_*_reset` (monolith `reset()` first statement, Reset.ts:114).
- **Why:** the passive trickle is gated on `highest_challenge_completions[3]>0`, so a pre-challenge-3 player has **zero** offering income. **Effort: medium** (the input-assembler is the real work; the per-tier award is then small). Also re-check whether `automation.offerings` vs `runes.rune_shards` is itself a dual-slice desync (sibling of H1).
